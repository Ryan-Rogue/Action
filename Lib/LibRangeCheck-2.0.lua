...
local minRangeCheck = function(unit) return not InCombatLockdown() and CheckInteractDistance(unit, 2) end
...
local checkers_Item = setmetatable({}, {
  __index = function(t, item)
    local func = function(unit)
      return not InCombatLockdown() and IsItemInRange(item, unit)
    end
    t[item] = func
    return func
  end
...
local checkers_Interact = setmetatable({}, {
  __index = function(t, index)
    local func = function(unit)
      if not InCombatLockdown() and CheckInteractDistance(unit, index) then
         return true
      end
    end
    t[index] = func

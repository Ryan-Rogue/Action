	...
	if AzeriteEssenceIsMajorUseable() then 
		...
		if MajorSpellName and Env.SpellCD(spellID) <= select(4, GetNetStats()) / 1000 + 0.025 and CombatTime("player") > 0 then 
			...
			if MajorSpellName == "Memory of Lucid Dreams" then
				...
				if not ShouldStop and (Env.UNITSpec("player", {66, 70, 263, 265, 266, 267, 62, 63, 64, 102}) or Env.UNITPW("player") <= 50) and (Env.Zone == "none" or Env.Unit(unitID):IsBoss() or UnitIsPlayer(unitID)) then 				
					-- PvP condition
